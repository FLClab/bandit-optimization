...
#    fname = "PSD95_sted10_exc20"
    ...
    for jm1 in range(Nb_images):
        ...
        sted_stack.append(sted[0][0])
    conf, _= microscope.acquire(config_conf)
    conf_stack.append(conf[0][0])

    conf_stack = np.moveaxis(np.array(conf_stack), 0, -1)
    sted_stack = np.moveaxis(np.array(sted_stack), 0, -1)
    #----------------------- SPLIT STED ---------------------------
    # Fin, phasor_analysis_fig = split_sted(sted_stack, config_split,  **config_split, return_analysis_fig=True )
    Fin = split_sted(sted_stack, config_split,  **config_split, return_analysis_fig=True )
    
    return Fin, sted_stack, conf_stack
