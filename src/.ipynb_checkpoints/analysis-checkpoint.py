...
def continuous_random_GS(x_mins=None, x_maxs=None,X=None, n_samples=100):
    #TODO: there is a bug...
    if X is not None:
...
def plot_regret(regret_data, optim_data,save_folders, surnames_dict, colors_dict=None, plot_all_curves=False):
    config = optim_data[save_folders[0]]['config']
    
    arr = np.array([len(optim_data[key]["X_list"]) for key in optim_data])
    all_same_repeats = np.all(arr == arr[0])
    
    x = range(1, config["optim_length"]+1)
...
#        print(algo,matrix.shape, mean.shape, std.shape)
        ...
        if colors_dict is not None:
            line = plt.plot(x, mean,color=colors_dict[algo], label=label)
        else:
...
def show_slices_nd(obj, optim_data=None, folder=None, X_sample=None, y_sample=None, algo=None, surnames_dict={}, idx=0, nb_iters=None):
...
#    save_folders = list(optim_data.keys())
    ...
    X = get_X_domain(obj.ndims_, n_points_default = 40)
    z = obj.evaluate(X)
    x_max = X[np.argmax(z)]
    ndims = obj.ndims_
    x_min_default = -1
...
def plot_optim_1D(obj, algo,X_list, y_list, config=None, idx=0, title=None, ax=None, max_iter=None, time_color=True):
